...
## v10 (current — FULL MONETIZATION ENGINE)
- ✅ **6 revenue products live** in Stripe:
  - Free (3 rewrites/session) → paywall pop
  - $5 — 10 Credits (pay-as-you-go, no expiry)
  - $19 — 50 Credits (Save 24%)
  - $20 — Guide PDF (kept)
  - $49/mo — Pro Subscription
  - $470/yr — Pro Annual
  - $299 — **Lifetime Founding Member** (limited 100, BEST VALUE badge)
- ✅ Switched from Emergent Stripe wrapper → **raw Stripe SDK** for `pro_month`/`pro_annual` subscriptions and promo code support
- ✅ `allow_promotion_codes=True` on every checkout — Stripe's UI lets buyers enter `COMEBACK29` etc.
- ✅ **Subscription mode** for monthly/yearly Pro (was one-time before)
- ✅ Hybrid: Emergent wrapper kept as fallback when `STRIPE_API_KEY=sk_test_emergent` so local dev still works
- ✅ **Paywall enforcement** on `/api/rewrite` — returns HTTP 402 + usage data after 3 free rewrites
- ✅ **PaywallModal.jsx** — 3-option upsell (10 credits / Pro / Lifetime) on hit, with Stripe-secure copy + 30-day money-back
- ✅ **Slack sale notifications** via `SLACK_WEBHOOK_URL` env — emoji-coded ping for every sale (motivates + tracks channels)
- ✅ **Credit decrement** logic — paid credits debit on each rewrite; Pro/Lifetime users skip
- ✅ **`/api/usage/{session_id}`** endpoint — frontend reads to know paywall state
- ✅ Pro/Lifetime entitlements grant unlimited

## Next session priorities
- [ ] **Switch Stripe to LIVE mode** on Railway (user action — replace `sk_test_emergent` with real `sk_live_...`)
- [ ] **Set up Slack webhook** in Slack admin → paste URL into Railway backend `SLACK_WEBHOOK_URL`
- [ ] **Set up Stripe webhook secret** for production payment events
- [ ] **Create Stripe Promo Code `COMEBACK29`** (40% off, 30 days valid) in Stripe Dashboard
- [ ] Create Stripe Promo Code `LIFETIME50` for first 50 lifetime buyers ($50 off → $249)
- [ ] Brokerage tier $499/mo (10 seats) — already mailto, can be Stripe later
- [ ] Affiliate program (30% recurring referrals)
- [ ] Telegram bot for buyer DMs
- [ ] Email magic-link login for cross-device entitlement persistence
- ✅ **Cinematic homepage demo video** (`/hero-demo.mp4`, 4.2 MB, 28s, 1080p, H.264 + AAC)
  - Generated by `backend/scripts/generate_hero_demo.py` — pulls 10 luxury Unsplash photos, renders through existing `video_engine.py` with cinematic music + 4 slide texts ("This isn't a house. It's the address you stop searching at.", etc.)
  - Output saved to `frontend/public/hero-demo.mp4` so it ships with the React build
  - Poster frame (mid-video keyframe) saved to `hero-demo-poster.jpg` — instant visual even on slow connections
  - `+faststart` flag applied so `moov` atom is at the start (web-streamable)
- ✅ **`AIVideoShowcase.jsx`** — full-bleed dark section between Hero and Marquee
  - Left: bold serif "Ten photos in. One cinematic reel out." headline + supporting copy + 10-thumbnail input grid + "RENDERED IN 8 SECONDS" tag
  - Right: 16:9 video frame with "AI-Generated · 0:28" pulsing badge, listworks.pro watermark, sound/play hover controls, vermillion glow shadow
  - IntersectionObserver pauses video when off-screen (saves bandwidth)
  - Bottom proof bar: 10 / 30s / 1080p / 9:16 stats
  - Fully responsive (12-col grid stacks on mobile)

## Next session priorities
- [ ] **Add `RESEND_API_KEY`, `EMAIL_FROM`, `SITE_URL` to Railway BACKEND vars** (user action)
- [ ] **Save to GitHub → Railway redeploy frontend** so `hero-demo.mp4` + poster + new component ship to listworks.pro
- [ ] Real Stripe live keys + live products
- [ ] Stripe coupon support so day-14 winback `COMEBACK29` actually applies 40%
- [ ] Telegram bot for buyer DMs / agent notifications
- [ ] Affiliate program (30% recurring referral)
- [ ] Resend webhook handler for delivery/bounce/complaint events

## v5 — Resend Email Drips + Custom Domain Live
